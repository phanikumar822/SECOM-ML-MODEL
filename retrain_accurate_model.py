...
# Calculate scale_pos_weight for handling imbalanced class without inflating probabilities artificially
...
# We use max_depth=3, learning_rate=0.03, scale_pos_weight for robust modeling
...
# Use Stratified K-Fold Cross Validation to calibrate optimal threshold
...
# Verify Baseline Chip Prediction
...
print("------------------------------------------")
print(f"VERIFICATION - Average Healthy Chip Failure Prob: {b_prob:.2%}")
if b_prob < best_threshold:
    print("VERIFICATION SUCCESSFUL: Average chip correctly predicts PASS (GOOD)!")
else:
    print("WARNING: Baseline chip still high prob!")

print("------------------------------------------")

# Save updated pipeline artifact
pipeline = {
    'imputer': imputer,
    'scaler': scaler,
    'selector': selector,
...
print("Model pipeline updated and saved to 'secom_defect_model.pkl'!")
